...
def clean_pg_array_field(value):
    ...
    if isinstance(value, list):
        ...
        # If it's neither a list nor a string, return an empty PostgreSQL array
        return '{}'

    # Format as PostgreSQL array
    return '{' + ','.join(cleaned_list) + '}'

def exists_in_db(self, title, release_date):
    """
    Checks if a movie with the given title and release date exists in the database.
    """
    query = """
        SELECT 1
        FROM allocine_movies
        WHERE title = %s AND release_date = %s
        LIMIT 1;
    """
    self.db_cursor.execute(query, (title, release_date))
