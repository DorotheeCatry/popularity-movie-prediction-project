...
class AllocineSpider(CrawlSpider):
    ...
    def parse_film(self, response):
        ...
        if 'Box Office' in header:
            ...
            yield scrapy.Request(boxoffice_url, meta={'meta_item': item}, callback=self.parse_boxoffice)
        else:
            item['box_office_fr'] = None
            item['box_office_us'] = None
            yield item
    ...
    def parse_boxoffice(self, response):
        ...
        # Box Office France
        box_office_fr_l1 = response.xpath('//h2[contains(text(), "Box Office France")]/parent::div/following-sibling::table[1]//tr[position() = 1]/td[2]/text()').get().strip()
        box_office_fr_l2 = response.xpath('//h2[contains(text(), "Box Office France")]/parent::div/following-sibling::table[1]//tr[position() = 2]/td[2]/text()').get().strip()
        
        if box_office_fr_l1:
            if box_office_fr_l1 < (box_office_fr_l2/2):
                item['box_office_fr'] = box_office_fr_l2
            else:
                item['box_office_fr'] = box_office_fr_l1
        else:
            item['box_office_fr'] = None
            
        
        # Box Office US
        box_office_us_l1 = response.xpath('//h2[contains(text(), "Box Office US")]/parent::div/following-sibling::table[1]//tr[position() = 1]/td[2]/text()').get().strip()
        box_office_us_l2 = response.xpath('//h2[contains(text(), "Box Office US")]/parent::div/following-sibling::table[1]//tr[position() = 2]/td[2]/text()').get().strip()

        if box_office_us_l2:
            if box_office_us_l1 < (box_office_us_l2/2):
                item['box_office_us'] = box_office_us_l2
            else:
                item['box_office_us'] = box_office_us_l1
