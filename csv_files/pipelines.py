...
class ReleaseDatabasePipeline:
    ...
    # Process each scraped item and insert into the database
    def process_item(self, item, spider):
        adapter = ItemAdapter(item)#from Scraping_twitter.config import DB_CONFIG
        ...
        if showings:
            showings = int(showings.replace(' ', '')) if showings else None   
            
        # Handle trailler
        trailer_views = adapter.get('trailer_views')
        if trailer_views:
            trailer_views = int(trailer_views.replace(' ', '')) if trailer_views else None
            
        trailer_date = adapter.get('trailer_date')
        if trailer_date:
            trailer_date = parse_date(trailer_date)
